...
contract StabilizeStrategyStablecoinArb is Ownable {
    ...
    uint256 public minReserve = 1000; // The minimum amount of stablecoin requested of the lowest priced coin
    uint256 constant minGain = 1e16; // Minimum amount of stablecoin gain (about 0.01 USD) before buying WETH and splitting it
    uint256 constant minDifference = 1000000000000000; // 0.001 difference in price, opportunity for gain
    ...
    function getCheapestCurveToken() internal returns (uint256) {
        // This will give us the ID of the cheapest token in the pool
        // We will estimate the return for trading 1000 DAI
        // The higher the return, the lower the price of the other token
        uint256 targetID = 0; // Our target ID is DAI first
        CurvePool pool = CurvePool(curveAddress);
        uint256 daiAmount = uint256(1000).mul(10**tokenList[0].decimals);
        uint256 highAmount = daiAmount;
        for(uint256 i = 1; i < tokenList.length; i++){
            uint256 estimate = pool.get_dy_underlying(tokenList[0].curveID, tokenList[i].curveID, daiAmount);
            ...
            if(estimate > highAmount){
                // This token is worth less than the DAI
                highAmount = estimate;
                targetID = i;
            }
        }
    ...
    function checkAndSwapTokens(bool _restrictSwaps) internal {
        ...
        if(profitable == true){
            ...
            if(_restrictSwaps == true){
                // Check if target ID has more than minimum reserve
                ...
                if(tokenList[targetID].token.balanceOf(address(this)) < _minReserveTarget){
                    // If less, we want to fill the reserve
            ...
            for(uint256 i = 0; i < length; i++){
                if(i != targetID){
                    ...
                    if(_restrictSwaps == true){
                        // When restrict swaps is true, DAI will only sell to sUSD and sUSD will only sell to DAI as these pairs swap prices frequently
                        if(i == 0){ // DAI
                            localTarget = 3; // sUSD
                        }else if(i == 3){ // sUSD
                            localTarget = 0; // DAI
    ...
    function isSellProfitable() internal view returns (bool) {
    ...
    function isSellProfitableView() external view returns (bool) {
        // This view will return whether a sell will be profitable for the strategy
        uint256 length = tokenList.length;
        uint256 targetID = 0;
        uint256 targetPrice = 0;
        uint256[4] memory tokenPrices;
        for(uint256 i = 0; i < length; i++){
            tokenPrices[i] = tokenList[i].price;
            // Get a more recent oracle price
            uint256 oraclePrice = oracleContract.getPrice(address(tokenList[i].token));
            if(oraclePrice > 0){
                tokenPrices[i] = oraclePrice;
            }
            if(targetPrice == 0 || tokenPrices[i] <= targetPrice){
                targetPrice = tokenPrices[i];
                targetID = i;
            }
        }
    ...
    function forceSwapTokens(bool _restrict) external onlyGovernance {
        // This is function that force trade tokens at anytime. It can only be called by governance
        updateTokenPrices(); // Update the prices based on an off-chain Oracle
        checkAndSwapTokens(_restrict);
    }
    ...
    // Change the minimum reserve
    // --------------------
    function startChangeMinReserve(uint256 _reserve) external onlyGovernance {
        _timelockStart = now;
        _timelockType = 9;
        _timelock_data_1 = _reserve;
    }
    
    function finishChangeMinReserve() external onlyGovernance timelockConditionsMet(9) {
        minReserve = _timelock_data_1;
    }
    // --------------------
}