...
contract Gauge is ReentrancyGuard {
    ...
    uint public constant MIN = 4000;
    uint public constant MAX = 25000;
    ...
    uint public totalDILL;
    mapping(address => uint) public balancesDILL;
    ...
    function rewardBalanceOf(address owner) public view returns (uint) {
        return _balances[owner].mul(balancesDILL[owner]).div(1e18);
    }
    
    function rewardTotalSupply() public view returns (uint) {
        return _totalSupply.mul(totalDILL).div(1e18);
    }
    ...
    function rewardPerToken() public view returns (uint256) {
        ...
        return
            rewardPerTokenStored.add(
                lastTimeRewardApplicable().sub(lastUpdateTime).mul(rewardRate).mul(1e18).div(rewardTotalSupply())
            );
    }

    function earned(address account) public view returns (uint256) {
        return rewardBalanceOf(account).mul(rewardPerToken().sub(userRewardPerTokenPaid[account])).div(1e18).add(rewards[account]);
    }
    ...
    function _deposit(uint amount, uint balance) internal nonReentrant updateReward(msg.sender) {
        ...
        totalDILL = DILL.totalSupply();
        balancesDILL[msg.sender] = DILL.balanceOf(msg.sender);
    ...
    function notifyRewardAmount(uint256 reward) external onlyDistribution updateReward(address(0)) {
        ...
        rewardRate = PICKLE.balanceOf(address(this)).div(DURATION);
