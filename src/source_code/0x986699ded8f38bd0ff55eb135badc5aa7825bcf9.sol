...
contract Resolver is AaveHelpers {
    function getPosition(address user, address[] memory tokens) public view returns(AaveUserTokenData[] memory, AaveUserData memory) {
        ...
        for (uint i = 0; i < tokens.length; i++) {
            address _token = tokens[i] == getEthAddr() ? getWethAddr() : tokens[i];
            tokensData[i] = getTokenData(AaveProtocolDataProvider(getAaveProtocolDataProvider()), user, _token, tokenPrices[i]);
        }
