...
contract ThirmProtocol is Ownable {
    ...
    function getTxn(string memory _txn) public view returns (bool _bool) {
        return transaction[_txn];
    }

    function getUserWithdrawLength() public view returns (uint256 arrayLength) {
    ...
    function registerWithdrawal(
        ...
        uint256 _amount
    ) public burnThirm returns (uint256 arrayLength) {
    ...
    modifier burnThirm() {
        require(
            feeAmount <= tokens["THIRM"].balanceOf(msg.sender),
            "no balance"
        );
        require(
            feeAmount <= tokens["THIRM"].allowance(msg.sender, address(this)),
            "no allowance"
        );
        tokens["THIRM"].burnFrom(msg.sender, feeAmount);
        _;
    }
}