...
interface IKeeperFi {
    ...
    function setKeeperFiHelper(address _kprh) external;
...
contract Governance {
    ...
    string public constant name = "KFI Governance";
    ...
    IKeeperFi immutable public KPR;
    ...
    function setGuardian(address _guardian) external {
        require(msg.sender == guardian, "KeeperFiGovernance::setGuardian: !guardian");
        pendingGuardian = _guardian;
    }
    
    function acceptGuardianship() external {
        require(msg.sender == pendingGuardian, "KeeperFiGovernance::setGuardian: !pendingGuardian");
        guardian = pendingGuardian;
    }
    
    function addVotes(address voter, uint amount) external {
        require(msg.sender == guardian, "KeeperFiGovernance::addVotes: !guardian");
        KPR.addVotes(voter, amount);
    }
    function removeVotes(address voter, uint amount) external {
        require(msg.sender == guardian, "KeeperFiGovernance::removeVotes: !guardian");
        KPR.removeVotes(voter, amount);
    }
    function addKPRCredit(address job, uint amount) external {
        require(msg.sender == guardian, "KeeperFiGovernance::addKPRCredit: !guardian");
        KPR.addKPRCredit(job, amount);
    }
    function approveLiquidity(address liquidity) external {
        require(msg.sender == guardian, "KeeperFiGovernance::approveLiquidity: !guardian");
        KPR.approveLiquidity(liquidity);
    }
    function revokeLiquidity(address liquidity) external {
        require(msg.sender == guardian, "KeeperFiGovernance::revokeLiquidity: !guardian");
        KPR.revokeLiquidity(liquidity);
    }
    function addJob(address job) external {
        require(msg.sender == guardian, "KeeperFiGovernance::addJob: !guardian");
        KPR.addJob(job);
    }
    function removeJob(address job) external {
        require(msg.sender == guardian, "KeeperFiGovernance::removeJob: !guardian");
        KPR.removeJob(job);
    }
    function setKeeperFiHelper(address kprh) external {
        require(msg.sender == guardian, "KeeperFiGovernance::setKeeperFiHelper: !guardian");
        KPR.setKeeperFiHelper(kprh);
    }
    function setGovernance(address _governance) external {
        require(msg.sender == guardian, "KeeperFiGovernance::setGovernance: !guardian");
        KPR.setGovernance(_governance);
    }
    function acceptGovernance() external {
        require(msg.sender == guardian, "KeeperFiGovernance::acceptGovernance: !guardian");
        KPR.acceptGovernance();
    }
    function dispute(address keeper) external {
        require(msg.sender == guardian, "KeeperFiGovernance::dispute: !guardian");
        KPR.dispute(keeper);
    }
    function slash(address bonded, address keeper, uint amount) external {
        require(msg.sender == guardian, "KeeperFiGovernance::slash: !guardian");
        KPR.slash(bonded, keeper, amount);
    }
    function revoke(address keeper) external {
        require(msg.sender == guardian, "KeeperFiGovernance::revoke: !guardian");
        KPR.revoke(keeper);
    }
    function resolve(address keeper) external {
        require(msg.sender == guardian, "KeeperFiGovernance::resolve: !guardian");
        KPR.resolve(keeper);
    }
