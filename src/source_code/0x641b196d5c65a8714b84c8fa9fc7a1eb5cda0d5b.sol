...
// File: contracts/MBCOREBTCVault.sol

pragma solidity ^0.6.10;

contract MBCOREBTCVault is Ownable {
    using SafeMath for uint256;
    ...
    // Constructor will set the address of MBCORE/BTC LP token
    constructor(address _LPToken, address _MBCOREToken, address _rewardPool, uint256 _dailyReward) Ownable() public {
    ...
    // 6500 blocks in average day --- decimals * MBCORE balance of rewardPool / blocks / 10000 * dailyReward (in hundredths of %) = rewardPerBlock
    ...
    // % of reward pool to be distributed each day
    ...
    function pendingRewards(address _staker) public view returns(uint256) {
        ...
        return user._LPDeposited.mul(_accMBCOREPerShare).div(1e18).sub(user._rewardDebt);
    }

    // Function that updates MBCORE/BTC LP pool
    ...
    // Function that lets user stake MBCORE/BTC LP
    function stakeLP(uint256 _amount) public {
        ...
        if(user._LPDeposited > 0) {
            uint256 _pendingRewards = user._LPDeposited.mul(accMBCOREPerShare).div(1e18).sub(user._rewardDebt);
        ...
        user._rewardDebt = user._LPDeposited.mul(accMBCOREPerShare).div(1e18);
    ...
    // Function that lets user unstake MBCORE/BTC LP in system
