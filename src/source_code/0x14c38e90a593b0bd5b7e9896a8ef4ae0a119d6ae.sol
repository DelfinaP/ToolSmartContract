...
contract DeflationaryERC20 is Context, IERC20, Ownable {
    ...
    //minimumsupply is 30000 WAV3. When the supply reach 30000, no more fee for transaction.
    uint256 private _minTotalSupply = 30000e18;
    ...
    // Transaction Fees:
    uint8 public txFee = 60; // artifical cap of 255 e.g. 25.5%
    ...
    function _transfer(
        ...
        // burn 20% txfee in each transfer
        if (
            transferToFeeDistributorAmount > 0 && feeDistributor != address(0)
        ) {
            uint256 burnAmount = transferToFeeDistributorAmount.div(10);
            _balances[feeDistributor] = _balances[feeDistributor].add(
                transferToFeeDistributorAmount.sub(burnAmount)
            );
            emit Transfer(
                sender,
                feeDistributor,
 ...
 */
contract Wav3Token is DeflationaryERC20 {
    constructor() public DeflationaryERC20("Wav3Token", "WAV3") {    
