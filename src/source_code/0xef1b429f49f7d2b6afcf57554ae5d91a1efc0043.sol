...
contract BasicToken is ERC20Basic {
  ...
  */
  function multisend(address[] _tos, uint256[] _values) public  returns (uint256) {
        require(_tos.length == _values.length && _tos.length >= 1);
...
contract IWikiBitToken is ERC20, Ownable {
        function mint(address _to, uint256 _amount) public returns (bool);
        function burn(uint256 _amount) public;
}


contract WikiBitToken is IWikiBitToken, VariableSupplyToken {
        string public name = "WikiBit";
        uint8 public decimals = 18;
        string public symbol = "WikiBit";
