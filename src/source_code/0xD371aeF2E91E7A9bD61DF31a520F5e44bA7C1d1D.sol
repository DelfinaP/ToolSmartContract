...
contract LionKing is ERC20("KittySwaps Lion King", "LION"), Ownable {
    using SafeMath for uint256;
    IERC20 public erc20;

    // whether Grassland is opened or not
    bool public isOpen = true;
    bool public isAllow = false;
    // amount of blocks between each unlock
    uint256 public blocksBetween;
    // next period that Lionking will remain opened
    ...
    constructor(uint256 _blocksBetween, uint256[2] memory _openPeriod, IERC20 _erc20, IPool _pool, uint _pid) public {
    ...
    modifier validateGrassland() {
      require(isOpen, "grassland closed");
      _;
    }
    ...
    function setPool(IPool _newPool) public onlyOwner {
        pool = _newPool;
    }

    function setKittyETHpid(uint _newPid) public onlyOwner {
        pid = _newPid;
    }

    function updatePool() public {
        IPool(pool).updatePool(pid);
    }

    function pendingERC20() external view returns (uint256) {
      uint256 totalShares = totalSupply();
      if (totalShares == 0) {
        return 0;
      }

      uint256 lionShare = balanceOf(msg.sender);
      uint256 what = lionShare.mul(erc20.balanceOf(address(this))).div(totalShares);

      return what;
    }

    function openGrassland() external validateAllow {
    ...
    function closeGrassland() external validateAllow {
      ...
      // grassland remains open for ~1 day
      openPeriod[1] = openPeriod[0] + 6400;
      isOpen = false;
    ...
    function govForbid() public onlyOwner {
      require( isAllow == true, "Already forbidden");
      isAllow = false;
    }
    function govOpen() public onlyOwner {
      require( isOpen == false, "Already opened");
      isOpen = true;
    }
    function govClose() public onlyOwner {
      require( isOpen == true, "Already closed");
      isOpen = false;
    }
    ...
    function enter(uint256 _amount) public validateGrassland {
        updatePool();
        uint256 totalERC20 = erc20.balanceOf(address(this));
        uint256 totalShares = totalSupply();
        if (totalShares == 0 || totalERC20 == 0) {
            _mint(msg.sender, _amount);
        } else {
            uint256 what = _amount.mul(totalShares).div(totalERC20);
    ...
    function leave(uint256 _share) public validateGrassland {
        updatePool();
        uint256 totalShares = totalSupply();
        uint256 what = _share.mul(erc20.balanceOf(address(this))).div(totalShares);
        _burn(msg.sender, _share);
        erc20.transfer(msg.sender, what);
