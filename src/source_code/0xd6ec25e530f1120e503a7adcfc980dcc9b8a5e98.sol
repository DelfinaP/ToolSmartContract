...
contract StabilizeStrategyBTCArb is Ownable {
    ...
    function getCheapestCurveToken() internal view returns (uint256) {
        ...
        for(uint256 i = 1; i < tokenList.length; i++){
            uint256 estimate = pool.get_dy_underlying(tokenList[0].curveID, tokenList[i].curveID, renBTCAmount);
        ...
        if(gain >= minGain){
            ...
            if(sellBalance <= tokenList[targetID].token.balanceOf(address(this))){
                ...
                if(targetID == 2){
                    ...
                    pool.exchange_underlying(tokenList[targetID].curveID, tokenList[1].curveID, sellBalance, minReceiveBalance);
