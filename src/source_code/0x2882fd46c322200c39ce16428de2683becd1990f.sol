...
contract Lock3rV1Volatility {
    ...
    ILock3rV1Oracle public constant LV1O = ILock3rV1Oracle(0xEcD33F8484C701aEb2FBabc96418c26B1D8250Ff);
    ...
    function price(address tokenIn, address tokenOut) public view returns (uint) {
        if (tokenIn == WETH) {
            return LV1O.current(WETH, 1e18, tokenOut);
        } else {
            uint _weth = LV1O.current(tokenIn, uint(10)**IERC20(tokenIn).decimals(), WETH);
            if (tokenOut == WETH) {
                return _weth;
            } else {
                return LV1O.current(WETH, _weth, tokenOut);
            }
        }
    }
    ...
    function rVol(address tokenIn, address tokenOut, uint points, uint window) public view returns (uint) {
        return vol(LV1O.sample(tokenIn, uint(10)**IERC20(tokenIn).decimals(), tokenOut, points, window));
    }
