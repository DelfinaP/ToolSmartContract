 ...
 */
contract CsfERC20 is Context, AccessControl, Ownable, ERC20Burnable, ERC20Pausable {
    using SafeMath for uint256;
     ...
     * beacuse he need to have almost DEFAULT_ADMIN_ROLE role to grant all role to the newOwner
     * Revoke all roles to the previous owner
     * Grants `DEFAULT_ADMIN_ROLE`, `MINTER_ROLE`, `PAUSER_ROLE` to the new owner
     */
    ...
    /**
     * @dev Throws if called by any account without `MINTER_ROLE`.
     */
    modifier onlyMinter() {
    ...
    /**
     * @dev Throws if called by any account without `PAUSER_ROLE`.
     */
    modifier onlyPauser() {
        require(hasRole(PAUSER_ROLE, _msgSender()), "CsfERC20: PAUSER role required");
        _;
    }
...
contract xBRKB is CsfERC20 {
    constructor() CsfERC20("Berkshire Hathaway Inc. Class B", "xBRKB", 1370952, 6) { }
}