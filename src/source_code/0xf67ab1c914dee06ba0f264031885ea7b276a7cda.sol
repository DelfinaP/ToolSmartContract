...
library SushiswapV2OracleLibrary {
    ...
    function currentCumulativePrices(
        ...
        price0Cumulative = ISushiswapV2Pair(pair).price0CumulativeLast();
        price1Cumulative = ISushiswapV2Pair(pair).price1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = ISushiswapV2Pair(pair).getReserves();
...
contract SushiswapV1Oracle {
    ...
    ISushiswapV2Router public constant UNI = ISushiswapV2Router(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);

    address public constant factory = 0xC0AEe478e3658e2610c5F7A4A2E1777cE9e4f2Ac;
    ...
    function pairFor(address tokenA, address tokenB) external pure returns (address) {
        return SushiswapV2Library.pairFor(factory, tokenA, tokenB);
    }
    
    function pairForWETH(address tokenA) external pure returns (address) {
        return SushiswapV2Library.pairFor(factory, tokenA, address(WETH));
    }
    ...
    function add(address tokenA, address tokenB) external {
        ...
        (uint price0Cumulative, uint price1Cumulative,) = SushiswapV2OracleLibrary.currentCumulativePrices(pair);
    ...
    function current(address tokenIn, uint amountIn, address tokenOut) external view returns (uint amountOut) {
        address pair = SushiswapV2Library.pairFor(factory, tokenIn, tokenOut);
        require(_valid(pair, periodSize.mul(2)), "SushiswapV1Oracle::quote: stale prices");
    ...
    function quote(address tokenIn, uint amountIn, address tokenOut, uint granularity) external view returns (uint amountOut) {
        address pair = SushiswapV2Library.pairFor(factory, tokenIn, tokenOut);
        require(_valid(pair, periodSize.mul(granularity)), "SushiswapV1Oracle::quote: stale prices");
        (address token0,) = SushiswapV2Library.sortTokens(tokenIn, tokenOut);
