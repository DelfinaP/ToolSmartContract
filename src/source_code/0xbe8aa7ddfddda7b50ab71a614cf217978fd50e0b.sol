...
contract dToken_proxy is ERC20, Iy_3dToken {
    ...
    constructor (address u_token, address y_token) ERC20(_proxy(IERC20(y_token).name()), _proxy(IERC20(y_token).symbol()), IERC20(y_token).decimals()) public {
        _u = u_token; _y = y_token;
        IERC20(_u).approve(_y, uint(-1));
    }
    ...
    function harvest() public {
        ...
        u.transfer(IUnisaveV2Factory(factory).feeTo(), delta);
        depositAll();
    }
}