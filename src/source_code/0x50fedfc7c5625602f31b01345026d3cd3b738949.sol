...
contract InstaToken is Initializable {
     ...
     * @param initialSupply_ Total supply of the token at genesis
     * @param account The initial account to grant all the tokens
     * @param minter_ The account with minting ability
     * @param mintingAllowedAfter_ The timestamp after which minting may occur
     * @param mintCap_ Cap on the percentage of totalSupply that can be minted at each mint
     * @param minimumTimeBetweenMints_ Minimum time between mints
     * @param _transferPaused Token transfer pause state
     */
    function initialize(string memory name_, string memory symbol_, uint initialSupply_, address account, address minter_, uint mintingAllowedAfter_, uint8 mintCap_, uint32 minimumTimeBetweenMints_, bool _transferPaused) public initializer {
        ...
        mintCap = mintCap_;
        minimumTimeBetweenMints = minimumTimeBetweenMints_;
    ...
    function pauseTransfer() external {
        require(msg.sender == minter, "Tkn::setMinter: only the minter can pause token transfer");
        transferPaused = true;
        emit TransferPaused(msg.sender);
    }
    ...
    function unpauseTransfer() external {
        require(msg.sender == minter, "Tkn::setMinter: only the minter can unpause token transfer");
    ...
    function mint(address dst, uint rawAmount) external {
        ...
        require(block.timestamp >= mintingAllowedAfter, "Tkn::mint: minting not allowed yet");
        ...
        require(amount <= SafeMath.div(SafeMath.mul(totalSupply, mintCap), 100), "Tkn::mint: exceeded mint cap");
        totalSupply = safe96(SafeMath.add(totalSupply, amount), "Tkn::mint: totalSupply exceeds 96 bits");
    ...
    function permit(address owner, address spender, uint rawAmount, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        ...
        require(now <= deadline, "Tkn::permit: signature expired");
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public {
        ...
        require(now <= expiry, "Tkn::delegateBySig: signature expired");
