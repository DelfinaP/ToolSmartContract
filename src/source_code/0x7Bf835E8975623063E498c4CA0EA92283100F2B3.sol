...
contract LiquidityRecipient is Controllable {
  ...
  */
  ...
  function stake(address pool, uint256 amount) external onlyGovernance {
    IERC20(uniLp).safeApprove(pool, 0);
    IERC20(uniLp).safeApprove(pool, amount);
    IRewardPool(pool).stake(amount);
  }

  function unstake(address pool) external onlyGovernance {
    IRewardPool(pool).exit();
  }

  function getReward(address pool) external onlyGovernance {
