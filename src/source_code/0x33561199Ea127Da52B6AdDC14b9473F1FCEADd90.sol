...
contract EthMen3D is ERC1155, Ownable {
    ...
    function unpack(address account, uint256 id, uint256 amount) public {
        require(account == _msgSender() || isApprovedForAll(account, _msgSender()), "Need operator approval for 3rd party burns.");
        EthMen ethMen = EthMen(0x1DD2c47496fBD9D38Ac9A884d15D816B062E1F6E);
        require(id % 2 == 0 && id != 0, "Not a Front Card provided");
        require(ethMen.balanceOf(account,id) >= amount, "Not enough Front Cards");
        require(ethMen.balanceOf(account,id+1) >= amount, "Not enough Back Cards");
        
        ethMen.burn(account, id, amount);
        ethMen.burn(account, id+1, amount);
        
        _customMint(account,id,amount);
        _customMint(account,id+1,amount);

        
    }
    
    function setTokenUri(uint256 id, string memory _metadata) public onlyOwner {
        require(id >= 2, "Id needs be 2 or greater");
    ...
    function totalBalance(address _owner) public view returns(uint256) {
        uint256 count = 0;
        for (uint256 id = 2; id <= highestId; id++) {
