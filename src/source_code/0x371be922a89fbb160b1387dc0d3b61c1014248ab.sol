...
contract IchiBuyer2 {
    ...
    IUniswapV2Factory public constant factory = IUniswapV2Factory(0x7dda55BB4403Aa2aa810d1eD49EFA614878e353b);
    address public constant staking = 0x70605a6457B0A8fBf1EEE896911895296eAB467E;
    address public constant ichi = 0x903bEF1736CDdf2A537176cf3C64579C3867A881;
    ...
    function _toWETH(address token, uint amountIn) internal returns (uint256) {
        if (token == ichi) {
            _safeTransfer(token, staking, amountIn);
            return 0;
        }
        if (token == weth) {
            _safeTransfer(token, factory.getPair(weth, ichi), amountIn);
            return amountIn;
        }
    ...
    function _toICHI(uint256 amountIn) internal {
        ...
        pair.swap(amount0Out, amount1Out, staking, new bytes(0));
    }
