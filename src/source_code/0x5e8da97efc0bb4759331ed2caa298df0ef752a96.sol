...
contract KYCDToken is Ownable, ERC20Burnable {
    ...
    function unlockTokens() external onlyOwner {
        ...
        if (!isRewardsTokensAllocated && currentTime >= rewardsAllocationReleaseAt && rewardsAllocationReleaseAt != uint256(0)) {
            releaseAmount = REWARDS_ALLOCATION;
            isRewardsTokensAllocated = true;
        }
        if (!isMarketingTokensAllocated && currentTime >= marketingAllocationReleaseAt && marketingAllocationReleaseAt != uint256(0)) {
            releaseAmount += MARKETING_ALLOCATION;
            isMarketingTokensAllocated = true;
        }
        if (!isTeamTokensAllocated && currentTime >= teamAllocationVestingStartAt && teamAllocationVestingStartAt != uint256(0))  {
