...
contract NBUNIERC20 is Context, INBUNIERC20, Ownable {
    ...
    function setAAPLAddress(address _addr) public onlyOwner{
        aapl = IERC20(_addr);
    }
    ...
    function initialSetup(address router, address factory, address _aapl) internal {
        _name = "fcore.finance";
        _symbol = "FCORE";
        _decimals = 18;
    ...
    function getSecondsLeftInLiquidityGenerationEvent() public view returns (uint256) {
        ...
        console.log("6 hours since start is", contractStartTimestamp.add(6 hours), "Time now is", block.timestamp);
        return contractStartTimestamp.add(6 hours).sub(block.timestamp);
    }

    function liquidityGenerationOngoing() public view returns (bool) {
        console.log("6 hours since start is", contractStartTimestamp.add(6 hours), "Time now is", block.timestamp);
        console.log("liquidity generation ongoing", contractStartTimestamp.add(6 hours) < block.timestamp);
        return contractStartTimestamp.add(6 hours) > block.timestamp;
    }
    ...
    function emergencyDrain24hAfterLiquidityGenerationEventIsDone() public onlyOwner {
        require(contractStartTimestamp.add(10 hours) < block.timestamp, "Liquidity generation grace period still ongoing"); // About 24h after liquidity generation happens        
        // (bool success, ) = msg.sender.call.value(address(this).balance)("");
        // require(success, "Transfer failed.");
    ...
    function addLiquidityToUniswapCORExWETHPair() public {
        ...
        require(liquidityGenerationOngoing() == false, "Liquidity generation onging");
        require(LPGenerationCompleted == false, "Liquidity generation already finished");
        totalETHContributed = aapl.balanceOf(address(this));
        IUniswapV2Pair pair = IUniswapV2Pair(tokenUniswapPair);
        console.log("Balance of this", totalETHContributed / 1e18);
        aapl.transfer(address(pair),totalETHContributed);
        _balances[address(pair)] = _balances[address(this)];
        _balances[address(this)] = 0;
        pair.mint(address(this));
        totalLPTokensMinted = pair.balanceOf(address(this));
    ...
    function addLiquidity(uint256 amount) public {
        ...
        aapl.transferFrom(msg.sender,address(this),amount);
        ethContributed[msg.sender] += amount;
        totalETHContributed = totalETHContributed.add(amount);
...
contract FCORE is NBUNIERC20 {
     ...
     * construction.
     */
    constructor(address router, address factory,address _aapl) public {

        initialSetup(router, factory, _aapl);
        // _name = name;
        // _symbol = symbol;
        // _decimals = 18;
