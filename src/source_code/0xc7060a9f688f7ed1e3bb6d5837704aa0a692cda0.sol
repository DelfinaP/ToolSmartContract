...
contract CheesePriceOracle is UniswapConfig {
    ...
    /// @notice The old observation for each symbolHash
    mapping(bytes32 => Observation) public oldLPObservations;

    /// @notice The new observation for each symbolHash
    mapping(bytes32 => Observation) public newLPObservations;
    ...
    event PairTokenSelected(string symbol, address tokenAddress, uint order);
    event LPBaseTokenConfig(string symbol, bytes32 indexed symbolHash);
    event PriceProtected(uint oldPrice, uint newPrice);
    ...
    UniswapConfig(sTokens_, underlyings_, symbolHashs_, baseUints_, priceSources_, fixedPrices_, uniswapMarkets_,
        ...
        for (uint i = 0; i < sTokens_.length; i++) {
            ...
            if (config.priceSource == PriceSource.REPORTER) {
                ...
                if (config.isLP) {
                    oldLPObservations[symbolHash].timestamp = block.timestamp;
                    newLPObservations[symbolHash].timestamp = block.timestamp;
                    oldLPObservations[symbolHash].acc = 0;
                    newLPObservations[symbolHash].acc = 0;
                } else {
    ...
    */
    function fetchLPAnchorPrice(string memory symbol, TokenConfig memory config, uint ethPrice) internal virtual returns (uint) {
        ...
        if (config.isUniswapReversed) {
            baseTokenAddress = pair.token1();
        } else {
            baseTokenAddress = pair.token0();
        }
        ERC20 baseToken = ERC20(baseTokenAddress);

        uint baseTokenConfigIndex = getUnderlyingIndex(baseTokenAddress);
        TokenConfig memory baseTokenConfig = getTokenConfig(baseTokenConfigIndex);
        uint baseTokenPrice;
        if (baseTokenConfig.symbolHash == ethHash) {
            baseTokenPrice = ethPrice;
        } else {
            baseTokenPrice = fetchAnchorPrice(symbol, baseTokenConfig, ethPrice);
            //baseTokenPrice = priceInternal(baseTokenConfig);
        }
        uint baseTokenBalance = baseToken.balanceOf(config.uniswapMarket);
        uint baseTokenValue = mul(baseTokenBalance, baseTokenPrice);
        uint totalValue = mul(baseTokenValue, 2);
        ...
        Observation memory newLPObservation = newLPObservations[symbolHash];

        oldLPObservations[symbolHash].timestamp = newLPObservation.timestamp;
        oldLPObservations[symbolHash].acc = newLPObservation.acc;

        if (newLPObservation.acc > 0 &&
            (anchorPrice > mul(newLPObservation.acc, 2) || anchorPrice < div(newLPObservation.acc, 2))) {
            emit PriceProtected(newLPObservation.acc, anchorPrice);
            anchorPrice = newLPObservation.acc;

        }

        newLPObservations[symbolHash].timestamp = block.timestamp;
        newLPObservations[symbolHash].acc = anchorPrice;
        emit UniswapWindowUpdated(config.symbolHash, newLPObservation.timestamp, block.timestamp, newLPObservation.acc, anchorPrice);
