...
interface IKeeperFi {
    ...
    function KPRH() external view returns (IKeeperFiHelper);
}
...
contract KeeperFiOracle {
    ...
    modifier keeper() {
        require(KFI.isMinKeeper(msg.sender, minKeep, 0, 0), "::isKeeper: keeper is not registered");
        _;
    }
    ...
    modifier upkeep() {
        ...
        require(KFI.isMinKeeper(msg.sender, minKeep, 0, 0), "::isKeeper: keeper is not registered");
        _;
        uint _received = KFI.KPRH().getQuoteLimit(_gasUsed.sub(gasleft()));
        KFI.receipt(address(KFI), address(this), _received);
    ...
    function _swap(uint _amount) internal returns (uint) {
        KFI.approve(address(UNI), _amount);
