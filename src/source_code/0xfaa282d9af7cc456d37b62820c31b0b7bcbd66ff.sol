...
contract XChanger is Ownable {
    ...
    function reverseQuote(
        ...
        uint256 inputAmount = uint256(-1);
        try UniswapV2Router02.getAmountsIn(returnAmount, path) returns (
            uint256[] memory amounts
        ) {
            if (amounts[0] < inputAmount) {
        ...
        if (toToken != WETH_ADDRESS) {
            ...
            try UniswapV2Router02.getAmountsIn(returnAmount, pathWETH) returns (
                uint256[] memory amountsWETH
            ) {
                if (amountsWETH[0] < inputAmount) {
                    inputAmount = amountsWETH[0];
                }
            } catch {}
        }
        
        require(inputAmount < uint256(-1), 'Reverse quote is wrong');
        
        // reverse-engineering reverse quote from Curve
        if (curveIndex[fromToken] > 0 && curveIndex[toToken] > 0)
            {
                ICurveFiCurve curve = ICurveFiCurve(CURVE_ADDRESS);
                // actual index is -1 lower
                uint returnCurve = curve.get_dy_underlying(getCurveIndex(fromToken), getCurveIndex(toToken), inputAmount);
                
                if (returnCurve > returnAmount) {
                    // means we can save a bit by using Curve here
                    inputAmount = returnAmount.mul(inputAmount).div(returnCurve);
                }
            }
