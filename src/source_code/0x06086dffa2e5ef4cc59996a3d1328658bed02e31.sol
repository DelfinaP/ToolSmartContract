...
contract FecoreDynamicStake2 is
    StakeTokenWrapper(0xfb8D6DB914A636a4437e1056e1780Ed3Ae3AAD5A),
    IGovernanceAddressRecipient
{
    IERC20 public FECORE = IERC20(0x6971fD17a3f82a7b9C957C82c6F4a5C8723Ba6C9);
    ...
    constructor() public {
        GovernanceAddress = _msgSender();
        FEE_ADDRESS = _msgSender();
    }

    function setFeeAddress(address _feeAddress) public onlyGovernanceAddress {
    ...
    function remainingReward() public view returns (uint256) {
        return FECORE.balanceOf(address(this));
    }
    ...
    function depositReward(uint256 amount) public onlyGovernanceAddress {
        ...
        FECORE.safeTransferFrom(_msgSender(), address(this), amount);
        initReward = amount;
    ...
    function getReward() public updateReward(_msgSender()) checkStart {
        ...
        if (reward > 0) {
            uint256 fee = reward.div(PERCENTS_DIVIDER).mul(FEE_RATE);
            rewards[_msgSender()] = 0;
            FECORE.safeTransfer(FEE_ADDRESS, fee);
            FECORE.safeTransfer(_msgSender(), reward.sub(fee));
