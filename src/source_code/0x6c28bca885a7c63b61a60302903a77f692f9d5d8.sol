...
contract AssGuarD is ReentrancyGuard {
    ...
    constructor(address _routerAddress) public {
        ...
        // Calc reward rate
        rewardRate = tokenAllocation.div(farmingDuration);
    ...
    function stake() external payable nonReentrant {
        ...
        // If eth amount = 0 then set initial price to 1 ETH = 69 ASS
        uint256 amountTokenDesired = ethAmount == 0 ? (amount * 69) : amount.mul(tokenAmount).div(ethAmount);
        // Mint borrowed ASS
    ...
    function unstake() external nonReentrant {
        ...
        require(farmingStartTimestamp + farmingDuration <= block.timestamp, 'You can unstake only after farming is over');
        require(accountInfos[msg.sender].balance > 0, 'Cannot unstake 0');

        uint256 amount = accountInfos[msg.sender].balance;
        uint256 reward = accountInfos[msg.sender].reward;
        
        // Reduce total supply
        _totalSupply = _totalSupply.sub(amount);
        // Reset balance & reward
        accountInfos[msg.sender].balance = 0;
    ...
    function lastRewardTimestamp() public view returns (uint256) {
        return Math.min(block.timestamp, endingTimestamp);
    }
    ...
    function _checkFarming() internal {
        require(farmingStartTimestamp != 0, 'Please wait until farming started');
        if (!farmingStarted) {
            farmingStarted = true;
            endingTimestamp = block.timestamp.add(farmingDuration);
