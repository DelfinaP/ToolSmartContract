...
// File: localhost/contracts/Zoracles.sol
...
// Zoracles with Governance
contract Zoracles is ERC20Upgradeable, OwnableUpgradeable {
    function initialize() public initializer {
        __ERC20_init("Zoracles", "ZORA");
        _setupDecimals(9);
        __Ownable_init();
    ...
    /// @notice Creates `_amount` token to `_to`. Must only be called by the timelock.
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "ZORA::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "ZORA::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "ZORA::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "ZORA::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying Zoras (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "ZORA::_writeCheckpoint: block number exceeds 32 bits"
        );
...
// File: localhost/contracts/Governance.sol
...
contract GovernorAlpha is Initializable {
    ...
    function propose(
        ...
        require(
            zora.getPriorVotes(msg.sender, sub256(block.number, 1)) >
                proposalThreshold(),
    ...
    function cancel(uint256 proposalId) public {
        ...
        require(
            msg.sender == guardian ||
                zora.getPriorVotes(proposal.proposer, sub256(block.number, 1)) <
                proposalThreshold(),
